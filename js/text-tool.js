 ...
 * FIXED: 
 * 1. Text always visible when not selected
 * 2. Text boxes preserved when zoom/scale changes (re-rendering)
 * 3. Proper z-index and visibility
 */
class TextTool {
  ...
  createTextBox(x, y, pageNum, layer) {
    ...
    const el = document.createElement('div');
    el.className = 'text-box selected'; // Start as selected
    el.dataset.id = id;
    el.contentEditable = true;
    el.style.left = x + 'px';
    el.style.top = y + 'px';
    ...
    // FIX: Ensure text is always visible with proper z-index
    el.style.zIndex = '100';
  ...
  resizeText(dx, dy) {
    ...
    // Proportional font size based on box height
  ...
  onBlur(id) {
    ...
    // FIXED: Only remove selected class, text remains visible
  ...
  setFontSize(size) {
    ...
    if (this.selectedText) {
      ...
      // Adjust box height to match new font size
